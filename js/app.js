...
(function () {
  ...
  function chartSeries(mode) {
    ...
    var reihen = Object.keys(per).filter(function (id) { return !profile(id).hidden; }).map(function (id) {
  ...
  function renderLigaKasse() {
    ...
    var darf = !!(online && d && d.darfBuchen);
    var art = UI.kasseArt === 'aus' ? 'aus' : 'ein';
    ...
    var zeilen = eintraege.map(function (e, i) {
      ...
      return '<div class="kasse-eintrag">' +
        ...
        (darf ? '<button class="weg" data-action="kasse-weg" data-id="' + e.id + '" aria-label="Buchung löschen">✕</button>' : '') +
        '</div>';
    });
    ...
    var beitragHtml = mitglieder.length ? '<div class="kasse-beitraege"><h3>Gründungsbeitrag ' + euro(konfig.beitrag) + ' pro Kopf</h3>' +
      mitglieder.map(function (m) {
        ...
        return '<div class="kb-zeile ' + (voll ? 'voll' : '') + '">' +
          ...
          (darf && !voll ? '<button class="btn ghost small" data-action="kasse-beitrag" data-id="' + m.id + '" data-name="' + esc(m.name) + '">Bezahlt</button>' : '') +
          '</div>';
      }).join('') + '</div>' : '';
    ...
    $('kasse-karte').innerHTML =
      ...
      (konfig.paypal
        ? '<a class="btn primary full kasse-paypal" href="' + esc(konfig.paypal) + '" target="_blank" rel="noopener">Per PayPal ins Vereinskonto einzahlen</a>' +
          '<p class="hint center">Der Kassenwart trägt die Einzahlung danach hier ein.</p>'
        : '') +
      beitragHtml +
      (darf
        ? '<div class="kasse-form">' +
            ...
            '</div>' +
            '<div class="zeile">' +
              '<input id="kasse-datum" type="date" value="' + heuteISO() + '" aria-label="Datum">' +
              kasseKategorieWahl(art, UI.kasseKategorie) +
            '</div>' +
            '<div class="zeile">' +
              '<input id="kasse-betrag" type="text" inputmode="decimal" placeholder="Betrag in €">' +
              '<input id="kasse-text" type="text" maxlength="80" placeholder="Beschreibung">' +
            '</div>' +
            '<div id="kasse-mitglied-zeile" class="' + (art === 'ein' && (UI.kasseKategorie || kasseKategorien('ein')[0]) === 'Mitgliedsbeiträge' ? '' : 'hidden') + '">' +
              '<select id="kasse-mitglied"><option value="">Mitglied (für den Beitrag)</option>' +
                mitglieder.map(function (m) { return '<option value="' + m.id + '">' + esc(m.name) + '</option>'; }).join('') +
              '</select></div>' +
            '<button class="btn primary full" data-action="kasse-buchen">Buchen</button>' +
            '<p class="hint" id="kasse-meldung"></p>' +
            '<details class="kasse-konfig"><summary>Kassenjahr und Anfangsbestand</summary>' +
              '<div class="zeile">' +
            ...
            '</details>' +
          '</div>'
        : (online ? '<p class="hint">Buchen kann nur der Kassenwart – alle anderen sehen hier das Kassenbuch.</p>'
                  : '<p class="hint">Zum Ansehen des Kassenbuchs bitte anmelden.</p>')) +
    ...
    switch (action) {
      ...
      case 'kasse-buchen': {
        ...
        window.DartSync.kasse.buchen({
          ...
          kasseDaten = d;
          UI.kasseArt = 'ein'; UI.kasseKategorie = null;
