...
(function () {
  ...
  function lineChart(series, label) {
    ...
    var axis = '<text x="' + L + '" y="' + (H - 8) + '" font-size="9" fill="#8d9aab">vor ' + (maxN - 1) + ' Spielen</text>' +
      '<text x="' + (W - R) + '" y="' + (H - 8) + '" text-anchor="end" font-size="9" fill="#8d9aab">letztes Spiel</text>';
  ...
  function renderBoards() {
    ...
    var chartLabel = mode === '501' ? '3-Dart-Average je Spiel' : 'MPR je Spiel';
  ...
  function findGame(kind, id) {
    if (kind === '501') {
      for (var i = 0; i < S.matches.length; i++) {
        if (S.matches[i].id === id) return { m: S.matches[i], start: S.settings.start, live: true };
      }
      ...
      case 'save-profile': {
        ...
        if (UI.overlay.id) {
          ...
          var np = { id: uid(), name: name, avatar: draft.avatar, created: Date.now(),
            hue: HUES[S.profiles.length % HUES.length] };
          S.profiles.push(np);
